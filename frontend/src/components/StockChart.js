...
const StockChart = memo(
  ({
    ...
    useEffect(() => {
      ...
      const start = new Date(Date.now() - 7 * 365 * 86400000).toISOString().split('T')[0];
      ...
      return (
        <div className="custom-tooltip">
          <p className="tooltip-label">
            {parseDateLocal(label).toLocaleDateString('en-US', {
              month: '2-digit',
              day: '2-digit',
              year: '2-digit',
            })}
    ...
    const renderCandleTooltip = useCallback(
      ({ payload, label }) => {
        if (!payload?.length) return null;
        ...
        return (
          <div className="custom-tooltip">
            ...
            <p>
              <span className="metric-name" style={{ color: 'var(--muted)' }}>
                O{' '}
              </span>
              <span className="metric-value">{parseFloat(d.Ticker_Open || 0).toFixed(2)}</span>
            </p>
            <p>
              <span className="metric-name" style={{ color: 'var(--green)' }}>
                H{' '}
              </span>
              <span className="metric-value">{parseFloat(d.Ticker_High || 0).toFixed(2)}</span>
            </p>
            <p>
              <span className="metric-name" style={{ color: 'var(--red)' }}>
                L{' '}
              </span>
              <span className="metric-value">{parseFloat(d.Ticker_Low || 0).toFixed(2)}</span>
            </p>
            <p>
              <span className="metric-name" style={{ color: isUp ? 'var(--green)' : 'var(--red)' }}>
                C{' '}
              </span>
              <span className="metric-value">{parseFloat(d.Ticker_Close || 0).toFixed(2)}</span>
            </p>
              ...
              return (
                <p key={m}>
                  <span className="metric-name" style={{ color }}>
                    {m.replace('Ticker_', '').replace(/_/g, ' ')}:{' '}
                  </span>
                  <span className="metric-value">
                    {d[m] != null ? parseFloat(d[m]).toFixed(2) : '—'}
                  </span>
                </p>
              );
            })}
                ...
                content={({ payload, label }) => {
                  ...
                  return (
                    <div className="custom-tooltip">
                      ...
                      <p>
                        <span className="metric-name" style={{ color: '#f97316' }}>
                          Return:{' '}
                        </span>
                        <span
                          className="metric-value"
                          style={{ color: v >= 100 ? '#22c55e' : '#ef4444' }}
                        >
