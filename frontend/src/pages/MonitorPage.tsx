...
function SectorHeatmap({ sectors }: { sectors: MonitorSector[] }) {
  const cols = [
    { metric: 'avg_change_pct' as const, label: '1D', max: 3 },
    { metric: 'avg_return_20d' as const, label: '20D', max: 12 },
    { metric: 'avg_rsi' as const, label: 'RSI', max: 0 },
    { metric: 'pct_above_200d' as const, label: '>200D', max: 0 },
  ]
  return (
        ...
        <table className="terminal-table compact">
          <thead>
            <tr>
              ...
              {cols.map((c) => <th key={c.metric} className="align-right">{c.label}</th>)}
            </tr>
          </thead>
          <tbody>
            {sectors.map((s) => (
